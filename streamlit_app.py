...
financing_ratio_percent = st.sidebar.slider("Financing Ratio (%)", min_value=0, max_value=100, value=90, step=1)
...
def calculate_star_rating(profit_ke: float) -> float:
    """Maps final accumulated profit (in k€) to a star rating (1.0 to 5.0)."""
    min_profit = 50.0  # 1 star
    max_profit = 10000.0 # 5 stars <= Updated to 10 Million

    if profit_ke <= min_profit:
        return 1.0
    if profit_ke >= max_profit:
        return 5.0

    # Linear interpolation between 1 and 5 stars for profits between 50k and 10M
    rating = np.interp(profit_ke, [min_profit, max_profit], [1.0, 5.0])
...
if run_button:
    ...
    try:
        ...
        # Extract final summary text and profit
        final_month_stats = summary_stats.iloc[-1]
        final_profit_mean = final_month_stats.get('AccumulatedProfit_mean', 0.0) # Use .get for safety
        summary_text = (
            f"After {total_simulation_months} months, estimated final assets: "
            f"{final_month_stats.get('Assets_mean', 0.0):.2f} k€ (Std Dev: {final_month_stats.get('Assets_std', 0.0):.2f} k€). "
            f"Median final assets: {final_month_stats.get('Assets_p50', 0.0):.2f} k€. "
            f"Mean Accumulated Profit: {final_profit_mean:.2f} k€."
        )

        st.subheader("Summary")
        # Display Star Rating first
        rating = calculate_star_rating(final_profit_mean)
